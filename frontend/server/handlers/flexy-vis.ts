...
export const getFlexyVisHandlers = (
  ...
  const get: Handler = async (req, res) => {
    ...
    try {
      ...
      res.status(500).send(`Failed to list Flexy-vis pods: ${details.message}`);
    }
  };
  ...
  const deleteHandler: Handler = async (req, res) => {
    ...
    try {
      ...
      const details = await parseError(err);
      console.error(`Failed to delete Flexy-vis app: ${details.message}`, details.additionalInfo);
      res.status(500).send(`Failed to delete Flexy-vis app: ${details.message}`);
    }
  };
